...
const CustomizeModal = ({ isOpen, onClose, product, onConfirm }) => {
  ...
  // Detecta limites com base no tamanho do açaí pelo título
  const getLimits = (title) => {
    const name = title.toLowerCase();
    if (name.includes('250ml')) return { creams: 1, fruits: 1, complements: 2 };
    if (name.includes('300ml')) return { creams: 3, fruits: 2, complements: 3 };
    if (name.includes('400ml')) return { creams: 3, fruits: 2, complements: 3 };
    if (name.includes('500ml')) return { creams: 3, fruits: 3, complements: 4 };
    if (name.includes('600ml')) return { creams: 3, fruits: 3, complements: 4 };
    if (name.includes('770ml')) return { creams: 2, fruits: 3, complements: 5 };
    if (name.includes('1 litro') || name.includes('1l')) return { creams: 2, fruits: 3, complements: 5 };
    return { creams: 2, fruits: 2, complements: 3 }; // Padrão de segurança
  };

  const limits = getLimits(product.title);

  // Opções disponíveis para o cliente escolher
  ...
  // Limpa as seleções ao mudar de produto
  useEffect(() => {
    setSelectedCreams([]);
    setSelectedFruits([]);
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-zinc-950 border border-fuchsia-700/30 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-grow">
              ...
              {availableCreams.map(cream => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={isDisabled}
                    onClick={() => handleToggle(cream, selectedCreams, setSelectedCreams, limits.creams)}
              ...
              {availableFruits.map(fruit => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={isDisabled}
                    onClick={() => handleToggle(fruit, selectedFruits, setSelectedFruits, limits.fruits)}
                    className={`p-3 rounded-xl border text-left text-xs font-semibold transition-all ${isChecked ? 'bg-fuchsia-700/20 border-fuchsia-600 text-gray-100' : 'bg-zinc-900 border-zinc-800 text-zinc-400 disabled:opacity-30'}`}
                  >
                    {fruit}
                  </button>
          ...
          <div>
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-sm font-bold uppercase text-gray-200 tracking-wider">Acompanhamentos / Complementos</h4>
              ...
              {availableComplements.map(comp => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    disabled={isDisabled}
                    onClick={() => handleToggle(comp, selectedComplements, setSelectedComplements, limits.complements)}
                    className={`p-3 rounded-xl border text-left text-xs font-semibold transition-all ${isChecked ? 'bg-fuchsia-700/20 border-fuchsia-600 text-gray-100' : 'bg-zinc-900 border-zinc-800 text-zinc-400 disabled:opacity-30'}`}
                  >
                    {comp}
                  </button>
        ...
        <div className="p-4 border-t border-zinc-800 bg-zinc-900 flex justify-end">
          <button
            onClick={handleSave}
            className="px-6 py-2.5 bg-lime-500 hover:bg-lime-400 text-zinc-950 font-bold font-['Barlow_Condensed'] uppercase tracking-wider rounded-xl transition-all"
          >
